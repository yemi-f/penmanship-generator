...
swatches and uploads them to B2 as public assets.
...
Card designs (Image A) are no longer part of this script — they're
generated per-card at request time from a user-supplied description,
not pre-generated as a static catalog. See app/service/cards.py.
"""
...
from app.service.prompts import STYLE_PROMPTS, build_swatch_prompt  # noqa: E402
from app.types.catalog import DEFAULT_STYLES  # noqa: E402

# Generation sizes must be multiples of 16 and >= 655360 total px (gpt-image-2
# constraints). Target canvas size (800x300) doesn't satisfy that, so we
# generate at the nearest compliant size and resize down to spec.
